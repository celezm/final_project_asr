...
private:
  ...
  void initializePublishers()
  {
    ...
    right_publisher_ = this->create_publisher<sensor_msgs::msg::Image>("/stereo/right/image_raw",
      10);
    left_rect_publisher_ =
      this->create_publisher<sensor_msgs::msg::Image>("/stereo/left/image_rect", 10);
    right_rect_publisher_ =
      this->create_publisher<sensor_msgs::msg::Image>("/stereo/right/image_rect", 10);
    left_camera_info_publisher_ =
      this->create_publisher<sensor_msgs::msg::CameraInfo>("/stereo/left/camera_info", 10);
    right_camera_info_publisher_ =
      this->create_publisher<sensor_msgs::msg::CameraInfo>("/stereo/right/camera_info", 10);
  }

  void initializeServices()
  {
    left_camera_info_manager_ = std::make_shared<camera_info_manager::CameraInfoManager>(this,
      "left_camera", left_camera_info_url_);
    right_camera_info_manager_ = std::make_shared<camera_info_manager::CameraInfoManager>(this,
      "right_camera", right_camera_info_url_);

    left_set_camera_info_service_ = this->create_service<sensor_msgs::srv::SetCameraInfo>(
            "left_camera/set_camera_info",
            std::bind(&StereoCameraNode::setCameraInfo, this, std::placeholders::_1,
      std::placeholders::_2, left_camera_info_manager_)
    );

    right_set_camera_info_service_ = this->create_service<sensor_msgs::srv::SetCameraInfo>(
            "right_camera/set_camera_info",
            std::bind(&StereoCameraNode::setCameraInfo, this, std::placeholders::_1,
      std::placeholders::_2, right_camera_info_manager_)
    );
  }
  ...
  void initiliazeRectificationMaps()
  {
    ...
    if (left_P.cols >= 3 && left_P.rows >= 3) {
      ...
      RCLCPP_ERROR(this->get_logger(),
        "Invalid projection matrix dimensions. Skipping calculation.");
      return;
    }
    ...
    cv::initUndistortRectifyMap(left_K, left_D, R1, P1, image_size, CV_32FC1, left_map1_,
      left_map2_);
    cv::initUndistortRectifyMap(right_K, right_D, R2, P2, image_size, CV_32FC1, right_map1_,
      right_map2_);

        // Check if maps are valid
    if (left_map1_.empty() || left_map2_.empty() || right_map1_.empty() || right_map2_.empty()) {
      RCLCPP_ERROR(this->get_logger(),
        "Failed to generate rectification maps. Skipping rectification.");
      return;
    }
  }
  ...
  void processFrame(const cv::Mat & frame)
  {
    ...
    if(publish_rectified_images_ || publish_raw_images_) {
      publishCameraInfo(left_camera_info_manager_->getCameraInfo(), "left_camera",
        left_camera_info_publisher_, current_time);
      publishCameraInfo(right_camera_info_manager_->getCameraInfo(), "right_camera",
        right_camera_info_publisher_, current_time);
    }
  ...
  void publishCameraInfo(
    const sensor_msgs::msg::CameraInfo & camera_info, const std::string & frame_id,
