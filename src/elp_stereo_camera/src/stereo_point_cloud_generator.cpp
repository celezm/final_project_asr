...
typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::msg::Image,
    sensor_msgs::msg::Image> ApproxSyncPolicy;

class StereoPointCloudGenerator : public rclcpp::Node
{
public:
  StereoPointCloudGenerator()
  : Node("stereo_point_cloud_generator_node"),
    ...
    cy_(0.0)
  {
    RCLCPP_INFO(this->get_logger(),
      "Iniciando Stereo Point Cloud Generator (Color, Ajustes Densidad)...");
    ...
    RCLCPP_INFO(this->get_logger(),
      "StereoSGBM params: numDisp=%d, blkSize=%d, uniqueRatio=%d, speckleWin=%d",
    ...
    left_camera_info_subscriber_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
            ...
            std::bind(&StereoPointCloudGenerator::left_camera_info_callback, this,
      std::placeholders::_1));
    right_camera_info_subscriber_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
            "/stereo/right/camera_info", 10,
            std::bind(&StereoPointCloudGenerator::right_camera_info_callback, this,
      std::placeholders::_1));
    ...
    sync_->registerCallback(std::bind(&StereoPointCloudGenerator::image_callback, this,
      std::placeholders::_1, std::placeholders::_2));
    point_cloud_publisher_ =
      this->create_publisher<sensor_msgs::msg::PointCloud2>("stereo/point_cloud_color", 10);
...
private:
  void left_camera_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
  {
    if (!left_info_ready_) {
      ...
      RCLCPP_INFO_ONCE(this->get_logger(), "Info cámara izquierda OK (fx=%.2f, cx=%.2f, cy=%.2f).",
        fx_, cx_, cy_);
    }
  }
  void right_camera_info_callback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
  {
    if (!right_info_ready_) {
      ...
      if (std::abs(p_fx) > 1e-6) {
        ...
        RCLCPP_INFO_ONCE(this->get_logger(), "Info cámara derecha OK (baseline=%.4f m).",
          baseline_);
      } else {
                // Error, fx es 0
      }
